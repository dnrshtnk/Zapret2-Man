...
STR_URL="https://raw.githubusercontent.com/dnrshtnk/Zapret2-Man/main/ListStrYou"
...
EXCLUDE_URL="https://raw.githubusercontent.com/dnrshtnk/Zapret2-Man/main/zapret-hosts-user-exclude.txt"
...
echo 'sh <(wget -O - https://raw.githubusercontent.com/dnrshtnk/Zapret2-Man/main/Zapret-Manager.sh)' > /usr/bin/zms; chmod +x /usr/bin/zms
# ==========================================
# Получение версии
# ==========================================
...
apk add unzip >/dev/null 2>&1 || { echo -e "\n${RED}Не удалось установить unzip!${NC}\n"; PAUSE; return; }; else opkg install unzip >/dev/null 2>&1 || { echo -e "\n${RED}Не удалось установить unzip!${NC}\n"; PAUSE; return; }; fi; fi
echo -e "${CYAN}Скачиваем архив ${NC}$FILE_NAME"; wget -q -U "Mozilla/5.0" -O "$FILE_NAME" "$LATEST_URL2" || { echo -e "\n${RED}Не удалось скачать $FILE_NAME${NC}\n"; PAUSE; return; }; echo -e "${CYAN}Распаковываем архив${NC}"
unzip -o "$FILE_NAME" >/dev/null; if [ "$PKG_IS_APK" -eq 1 ]; then PKG_PATH="$TMP_SF/apk"; for PKG in "$PKG_PATH"/zapret2*; do [ -f "$PKG" ] || continue; echo "$PKG" | grep -q "luci" && continue; install_pkg "$(basename "$PKG")" "$PKG" || return; done
for PKG in "$PKG_PATH"/luci*; do [ -f "$PKG" ] || continue; install_pkg "$(basename "$PKG")" "$PKG" || return; done; else PKG_PATH="$TMP_SF"; for PKG in "$PKG_PATH"/zapret2_*.ipk; do [ -f "$PKG" ] || continue; install_pkg "$(basename "$PKG")" "$PKG" || return; done
for PKG in "$PKG_PATH"/luci-app-zapret2_*.ipk; do [ -f "$PKG" ] || continue; install_pkg "$(basename "$PKG")" "$PKG" || return; done; fi; echo -e "${CYAN}Удаляем временные файлы${NC}"; cd /
rm -rf "$TMP_SF" /tmp/*.ipk /tmp/*.zip /tmp/*zapret* 2>/dev/null; echo -e "Zapret2 ${GREEN}установлен!${NC}\n"; [ "$NO_PAUSE" != "1" ] && PAUSE; return 0; }
# ==========================================
...
6) toggle_finland_hosts; continue;; 7) switch_Dv; continue;; 8) echo -e "\n${YELLOW}Настройки Discord для Zapret2${NC}\n\n${RED}Функционал в разработке!${NC}\n"; PAUSE; continue;; *) return;; esac; fi; [ ! -f /etc/init.d/zapret ] && { echo -e "\n${RED}Zapret не установлен!${NC}\n"; PAUSE; continue; }
...
for pid in $(pgrep -f /opt/zapret 2>/dev/null); do kill -9 "$pid" 2>/dev/null; done; echo -e "${CYAN}Удаляем пакеты${NC}"; pkg_remove zapret; pkg_remove zapret2; pkg_remove luci-app-zapret; pkg_remove luci-app-zapret2; echo -e "${CYAN}Удаляем временные файлы${NC}"
...
# Меню стратегий
# ==========================================
menu_str() { while true; do show_current_strategy; RKN_Check; clear; echo -e "${MAGENTA}Меню стратегий${NC}\n"
# Проверка для Zapret
if [ -f /etc/init.d/zapret ]; then ZAPRET1_INST="${GREEN}установлен${NC}"; else ZAPRET1_INST="${RED}не установлен${NC}"; fi
# Проверка для Zapret2
if [ -f /etc/init.d/zapret2 ]; then ZAPRET2_INST="${GREEN}установлен${NC}"; else ZAPRET2_INST="${RED}не установлен${NC}"; fi
...
echo -e "${CYAN}8) ${GREEN}Меню стратегий для Zapret2${NC} ${RED}(в разработке)${NC}"
...
# Главное меню
# ==========================================
# Подменю выбора версии Zapret/Zapret2
...
if [ "$NFQ_ALL" -gt 0 ]; then [ "$NFQ_RUN" -eq "$NFQ_ALL" ] && NFQ_CLR=$GREEN || NFQ_CLR=$RED; NFQ_STAT="${NFQ_CLR}[${NFQ_RUN}/${NFQ_ALL}]${NC}"; fi
if [ -f /etc/init.d/zapret ]; then /etc/init.d/zapret status >/dev/null 2>&1 && ZAPRET_STATUS="${GREEN}запущен $NFQ_STAT${NC}" || ZAPRET_STATUS="${RED}остановлен${NC}"; else ZAPRET_STATUS=""; fi
# Определяем действие для пункта 1
if [ ! -f /etc/init.d/zapret ]; then Z_ACTION_TEXT="Установить"; Z_ACTION_FUNC="install_Zapret"
elif [ "$INST_VER" = "$ZAPRET_VERSION" ] || [ "$INST_VER" = "$ZAPRET2_VERSION" ]; then Z_ACTION_TEXT="Удалить"; Z_ACTION_FUNC="uninstall_zapret"
...
1) version_submenu;; 2) pgrep -f /opt/zapret >/dev/null 2>&1 && stop_zapret || start_zapret;; 3) menu_str;; 4) DoH_menu;; 5) Discord_menu;; 6) backup_menu;; 7) menu_hosts;; 8) zapret_key;; 0) sys_menu;; *) echo; exit 0;; esac; }; while true; do show_menu; done
