...
get_versions() { 
    ...
    NFQ_STAT=""; if [ "$NFQ_ALL" -gt 0 ]; then
        [ "$NFQ_RUN" -eq "$NFQ_ALL" ] && NFQ_CLR="$GREEN" || NFQ_CLR="$RED"; NFQ_STAT="${NFQ_CLR}[${NFQ_RUN}/${NFQ_ALL}]${NC}"
    fi
    if [ -f /etc/init.d/zapret ]; then 
        /etc/init.d/zapret status >/dev/null 2>&1 && ZAPRET_STATUS="${GREEN}запущен $NFQ_STAT${NC}" || ZAPRET_STATUS="${RED}остановлен${NC}"
    else ZAPRET_STATUS=""; fi
    # Цвет статуса если установлена любая из версий
    if [ "$INSTALLED_VER" = "$ZAPRET_VERSION" ] || [ "$INSTALLED_VER" = "$ZAPRET2_VERSION" ]; then 
        INST_COLOR=$GREEN
    else 
        INST_COLOR=$RED
    fi
...
install_Zapret() { 
   ...
   if [ "$VER_TYPE" = "2" ]; then
    ...
    LATEST_URL="https://github.com/remittor/zapret-openwrt/releases/download/v${ZAPRET2_VERSION}/zapret2_v${ZAPRET2_VERSION}_${LOCAL_ARCH}.zip"
    else
        TARGET_VERSION="$ZAPRET_VERSION"
        LATEST_URL="https://github.com/remittor/zapret-openwrt/releases/download/v${ZAPRET_VERSION}/zapret_v${ZAPRET_VERSION}_${LOCAL_ARCH}.zip"
    fi

    # Проверка установки именно этой версии
    if [ "$INSTALLED_VER" = "$TARGET_VERSION" ]; then 
        echo -e "\n${GREEN}Zapret версии $TARGET_VERSION уже установлен!${NC}\n"
        [ "$NO_PAUSE" != "1" ] && PAUSE
        return
    fi
    
    [ "$NO_PAUSE" != "1" ] && echo
    echo -e "${MAGENTA}Устанавливаем ZAPRET v${TARGET_VERSION}${NC}"
    if [ -f /etc/init.d/zapret ]; then 
    ...
    rm -f "$TMP_SF"/* 2>/dev/null; cd "$TMP_SF" || return
...
zapret_key() { 
    ...
    get_versions
    uninstall_zapret "1"
    install_Zapret "1" "1" # По умолчанию версия 1
...
# ==========================================
show_menu() { 
    get_versions; get_doh_status; show_current_strategy; RKN_Check; mkdir -p "$TMP_SF"
    CURR=$(curr_MIR); clear
    echo -e "╔════════════════════════════════════╗
...
${DGRAY}by StressOzz v$ZAPRET_MANAGER_VERSION${NC}"
    ...
    elif [ "$INSTALLED_VER" = "$ZAPRET_VERSION" ] || [ "$INSTALLED_VER" = "$ZAPRET2_VERSION" ]; then Z_ACTION_TEXT="Удалить"; Z_ACTION_FUNC="uninstall_zapret"
    ...
    echo -e "\n${CYAN}1) ${GREEN}Управление Zapret${NC}
...
${CYAN}0) ${GREEN}Системное меню${NC}"
    ...
    case "$choice" in 
        888) echo; uninstall_zapret "1"; install_Zapret "1"; curl -fsSL https://raw.githubusercontent.com/StressOzz/Test/refs/heads/main/zapret -o "$CONF"; hosts_add "$ALL_BLOCKS"; rm -f "$EXCLUDE_FILE"; wget -q -U "Mozilla/5.0" -O "$EXCLUDE_FILE" "$EXCLUDE_URL"; ZAPRET_RESTART; PAUSE;;
        1) 
            clear
            echo -e "${MAGENTA}Выберите версию Zapret${NC}\n"
            echo -e "${CYAN}1) ${GREEN}Zapret v${ZAPRET_VERSION}${NC}"
            echo -e "${CYAN}2) ${GREEN}Zapret v${ZAPRET2_VERSION}${NC}"
            echo -e "${CYAN}Enter) ${GREEN}Назад${NC}\n"
            echo -ne "${YELLOW}Выберите версию:${NC} "
            read ver_choice
            case "$ver_choice" in
                1) TARGET_VER="$ZAPRET_VERSION"; V_TYPE="1";;
                2) TARGET_VER="$ZAPRET2_VERSION"; V_TYPE="2";;
                *) continue;;
            esac
            get_versions
