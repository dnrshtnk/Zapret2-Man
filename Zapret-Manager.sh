...
# =========================================
ZAPRET_MANAGER_VERSION="9.1"; ZAPRET_VERSION="72.20260226"; ZAPRET2_VERSION="0.9.20260226"; STR_VERSION_AUTOINSTALL="v1"
...
if [ "$PKG_IS_APK" -eq 1 ]; then INSTALLED_VER=$(apk info -v | grep '^zapret-' | head -n1 | cut -d'-' -f2 | sed 's/-r[0-9]\+$//')
INSTALLED_VER2=$(apk info -v | grep '^zapret2-' | head -n1 | cut -d'-' -f2 | sed 's/-r[0-9]\+$//')
else INSTALLED_VER=$(opkg list-installed zapret 2>/dev/null | awk '{sub(/-r[0-9]+$/, "", $3); print $3}')
INSTALLED_VER2=$(opkg list-installed zapret2 2>/dev/null | awk '{sub(/-r[0-9]+$/, "", $3); print $3}'); fi
[ -z "$INSTALLED_VER" ] && INSTALLED_VER="не найдена"; [ -z "$INSTALLED_VER2" ] && INSTALLED_VER2="не найдена"
...
else ZAPRET_STATUS=""; fi; [ "$INSTALLED_VER" = "$ZAPRET_VERSION" ] && INST_COLOR=$GREEN || INST_COLOR=$RED
[ "$INSTALLED_VER2" = "$ZAPRET2_VERSION" ] && INST_COLOR2=$GREEN || INST_COLOR2=$RED
INSTALLED_DISPLAY=${INSTALLED_VER:-"не найдена"}; INSTALLED_DISPLAY2=${INSTALLED_VER2:-"не найдена"}; }
...
install_Zapret() { local TYPE=${1:-1}; local NO_PAUSE=${2:-0}; mkdir -p "$TMP_SF"; get_versions
if [ "$TYPE" = "2" ]; then VER_USE="$ZAPRET2_VERSION"; PKG_BASE="zapret2"; URL_BASE="zapret2_v"; URL_USE="$LATEST_URL2"
else VER_USE="$ZAPRET_VERSION"; PKG_BASE="zapret"; URL_BASE="zapret_v"; URL_USE="$LATEST_URL"; fi
[ "$INSTALLED_VER" = "$VER_USE" ] && [ "$TYPE" = "1" ] && { echo -e "
${GREEN}Zapret уже установлен!${NC}
"; [ "$NO_PAUSE" != "1" ] && PAUSE; return; }
[ "$INSTALLED_VER2" = "$VER_USE" ] && [ "$TYPE" = "2" ] && { echo -e "
${GREEN}Zapret2 уже установлен!${NC}
…rm -rf "$TMP_SF" /tmp/*.ipk /tmp/*.zip /tmp/*zapret* 2>/dev/null; echo -e "Zapret ${TYPE} ${GREEN}установлен!${NC}
"; [ "$NO_PAUSE" != "1" ] && PAUSE; }
# ==========================================
...
"; get_versions; uninstall_zapret 1 "1"; install_Zapret 1 "1"
...
uninstall_zapret() { local TYPE=${1:-1}; local NO_PAUSE=$2; [ "$NO_PAUSE" != "1" ] && echo; echo -e "${MAGENTA}Удаляем ZAPRET ${TYPE}${NC}
${CYAN}Останавливаем ${NC}zapret"; /etc/init.d/zapret stop >/dev/null 2>&1; echo -e "${CYAN}Убиваем процессы${NC}"
for pid in $(pgrep -f /opt/zapret 2>/dev/null); do kill -9 "$pid" 2>/dev/null; done; echo -e "${CYAN}Удаляем пакеты${NC}"
if [ "$TYPE" = "2" ]; then pkg_remove zapret2; pkg_remove luci-app-zapret2; else pkg_remove zapret; pkg_remove luci-app-zapret; fi
...
nft list tables 2>/dev/null | awk '{print $2}' | grep -E '(zapret|ZAPRET)' | while read t; do [ -n "$t" ] && nft delete table "$t" 2>/dev/null; done; rm -rf -- "$TMP_SF"; echo -e "Zapret ${TYPE} ${GREEN}удалён!${NC}
"; [ "$NO_PAUSE" != "1" ] && PAUSE; }

# Меню установки
# ==========================================
install_menu() { while true; do get_versions; clear; echo -e "${MAGENTA}Управление установкой Zapret${NC}
"
echo -e "${YELLOW}Zapret 1:${NC} ${INST_COLOR}$INSTALLED_DISPLAY${NC} | ${YELLOW}Zapret 2:${NC} ${INST_COLOR2}$INSTALLED_DISPLAY2${NC}
"
if [ "$INSTALLED_VER" != "не найдена" ]; then ACT1="${GREEN}Удалить${NC}"; else ACT1="${GREEN}Установить${NC}"; fi
if [ "$INSTALLED_VER2" != "не найдена" ]; then ACT2="${GREEN}Удалить${NC}"; else ACT2="${GREEN}Установить${NC}"; fi
echo -e "${CYAN}1) ${ACT1} Zapret 1 (v${ZAPRET_VERSION})
${CYAN}2) ${ACT2} Zapret 2 (v${ZAPRET2_VERSION})
${CYAN}Enter) ${GREEN}Выход в главное меню${NC}
${YELLOW}Выберите пункт:${NC} " && read choiceIM
case "$choiceIM" in 1) if [ "$INSTALLED_VER" != "не найдена" ]; then uninstall_zapret 1; else install_Zapret 1; fi;;
2) if [ "$INSTALLED_VER2" != "не найдена" ]; then uninstall_zapret 2; else install_Zapret 2; fi;;
*) return;; esac; done; }
# ==========================================
...
if [ ! -f /etc/init.d/zapret ]; then Z_ACTION_TEXT="Установить"; elif [ "$INSTALLED_VER" = "$ZAPRET_VERSION" ]; then Z_ACTION_TEXT="Удалить"; else Z_ACTION_TEXT="Обновить"; fi
...
case "$choice" in 888) echo; uninstall_zapret 1 "1"; install_Zapret 1 "1"; curl -fsSL https://raw.githubusercontent.com/StressOzz/Test/refs/heads/main/zapret -o "$CONF"; hosts_add "$ALL_BLOCKS"; rm -f "$EXCLUDE_FILE"; wget -q -U "Mozilla/5.0" -O "$EXCLUDE_FILE" "$EXCLUDE_URL"; ZAPRET_RESTART; PAUSE;;
1) install_menu;; 2) pgrep -f /opt/zapret >/dev/null 2>&1 && stop_zapret || start_zapret;; 3) menu_str;; 4) DoH_menu;; 5) Discord_menu;; 6) backup_menu;; 7) menu_hosts;; 8) zapret_key;; 0) sys_menu;; *) echo; exit 0;; esac; }; while true; do show_menu; done
